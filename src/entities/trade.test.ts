import { CurrencyAmount, Ether, Percent, Price, sqrt, Token, TradeType, WETH9 } from '@jingofi/sdk-core'
...
describe('Trade', () => {
  ...
  const token3 = new Token(9745, '0x0000000000000000000000000000000000000004', 18, 't3', 'token3')
  ...
  const pool_weth_0 = v2StylePool(
    CurrencyAmount.fromRawAmount(WETH9[9745], JSBI.BigInt(100000)),
  ...
  describe('#fromRoute', () => {
    it('can be constructed with ETHER as input', async () => {
      const trade = await Trade.fromRoute(
        ...
        CurrencyAmount.fromRawAmount(Ether.onChain(9745), JSBI.BigInt(10000)),
  ...
  describe('#fromRoutes', () => {
    it('can be constructed with ETHER as input with multiple routes', async () => {
      const trade = await Trade.fromRoutes<Ether, Token, TradeType.EXACT_INPUT>(
        [
          {
            amount: CurrencyAmount.fromRawAmount(Ether.onChain(9745), JSBI.BigInt(10000)),
    ...
    it('can be constructed with ETHER as output with multiple routes', async () => {
      const trade = await Trade.fromRoutes<Token, Ether, TradeType.EXACT_OUTPUT>(
        [
          {
            amount: CurrencyAmount.fromRawAmount(Ether.onChain(9745), JSBI.BigInt(4000)),
            route: new Route([pool_weth_0], token0, ETHER)
          },
          {
            amount: CurrencyAmount.fromRawAmount(Ether.onChain(9745), JSBI.BigInt(6000)),
  ...
  describe('#bestTradeExactIn', () => {
    ...
    it('works for ETHER currency input', async () => {
      const result = await Trade.bestTradeExactIn(
        ...
        CurrencyAmount.fromRawAmount(Ether.onChain(9745), JSBI.BigInt(100)),
        token3
      )
      expect(result).toHaveLength(2)
      expect(result[0].inputAmount.currency).toEqual(ETHER)
      expect(result[0].swaps[0].route.tokenPath).toEqual([WETH9[9745], token0, token1, token3])
      expect(result[0].outputAmount.currency).toEqual(token3)
      expect(result[1].inputAmount.currency).toEqual(ETHER)
      expect(result[1].swaps[0].route.tokenPath).toEqual([WETH9[9745], token0, token3])
    ...
    it('works for ETHER currency output', async () => {
      ...
      expect(result[0].swaps[0].route.tokenPath).toEqual([token3, token0, WETH9[9745]])
      expect(result[0].outputAmount.currency).toEqual(ETHER)
      expect(result[1].inputAmount.currency).toEqual(token3)
      expect(result[1].swaps[0].route.tokenPath).toEqual([token3, token1, token0, WETH9[9745]])
